...
module Presenter
  class IssuesAuthoredByMePresenter < Presenter::PresenterBase
    ...
    def initialize
      ...
      @title_text = 'Issues Authored By Me'
      @issue_list = []
      @area = nil
      @should_publish_on_load = true
      @tab_id = 'issues_authored_by_me'
      @is_tab_active = true
      @issue_table = nil
      fetch_issue_list
    end
    ...
    def fetch_issue_list(silent: false)
      ...
      @issue_list = build_list_view_array @gitlab_service.open_issues_authored_by_me
    ...
    def update(message)
      if message.is_a?(Model::EventMessages::ForceReloadMessage)
        console_log 'force reload of issues authored by me'
    ...
    def on_load
      ...
      @message_bus.publish_message(message: Model::EventMessages::StatusBarUpdateQueueMessage.new(message: 'Issues authored by me loaded', display_for: 0, is_priority: true))
    ...
    def status_text
      ...
      @status_text = if issue_list.empty?
                       'You have not authored any issues.'
                     else
                       "You have authored #{issue_list.size} issue#{issue_list.size == 1 ? '' : 's'}."
                     end
    end
